...
async function loadNasabahDashboard() {
    ...
    (trx||[]).forEach(t=>{tb+=t.berat_kg; lc.innerHTML+=`<div class="flex justify-between items-center p-4 bg-white rounded-xl border border-gray-100 shadow-sm"><div><p class="font-bold text-gray-900">${t.jenis_sampah?.nama_sampah}</p><p class="text-xs text-gray-500">${new Date(t.tanggal_transaksi).toLocaleDateString('id-ID')} • ${t.berat_kg} ${t.jenis_sampah?.satuan}</p></div><div class="text-right"><p class="font-bold text-emerald-700">${formatRupiah(t.total_harga)}</p><span class="text-[10px] px-2 py-0.5 rounded-full ${t.status_bayar==='dibayar'?'bg-green-100Siap ges! Ini **Full Code `addon.js` FINAL** yang sudah saya perbaiki total.

Perbaikan di versi ini:
1.  **Fix List Pending Kosong:** Fungsi `loadPendingUsersAdmin` sekarang menggunakan `.select('*, email')` untuk memastikan data email terambil dengan benar (karena email ada di metadata auth, bukan kolom fisik tabel profiles).
2.  **Fix Auto-Refresh State:** Fitur "Kembali ke Halaman Terakhir" sudah terintegrasi penuh di fungsi `showSection`, `switchAdminTab`, dan `initApp`.
3.  **Integrasi Account Manager:** Semua fungsi pembuatan user, approval, dan reset password sudah terhubung rapi dengan `account-manager.js`.

Silakan copy-paste **SEMUA** kode di bawah ini ke file `addon.js` kamu di GitHub:
